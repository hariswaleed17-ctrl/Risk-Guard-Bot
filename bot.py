...
# CONFIG — EDIT THESE
# =========================
TICKER = "KXBTC15M-26MAR030400-00"
POLL_SECONDS = 2

# SAFETY DEFAULTS (start here)
DRY_RUN = False      # True = no real orders
ARM_LIVE = True     # must be True to send real orders

# Money / risk
BUDGET_USD = 1.00              # approx capital used per entry
MAX_DAILY_LOSS_USD = .25      # hard stop for the day (set small while testing)
FEES_SLIPPAGE_BUFFER_USD = 0.20  # conservative “friction” per round-trip
...
ALLOW_FLIP_ON_LOSS = False      # OFF (flipping = turbo-loss)
ALLOW_REENTER_ON_PROFIT = True  # OK, but still risky

COOLDOWN_SECONDS = 20
MAX_TRADES_PER_SESSION = 20

# Order pricing aggressiveness
ENTRY_PAY_UP_CENTS = 1       # buy at bid+1
EXIT_SLIP_CENTS = 1          # sell at bid-1
EXIT_RETRIES = 6
EXIT_RETRY_SLEEP = 1.0
...
def order_filled(order_json: Dict[str, Any]) -> bool:
    # robust-ish: check status and filled counts
...
def extract_avg_fill_price_cents(order_json: Dict[str, Any], side: str) -> Optional[int]:
    ...
    for k in ("avg_fill_price", "average_fill_price", "avg_price", "fill_price"):
        ...
        if v is None:
            continue
        try:
            # could be dollars string or cents int; normalize to cents
    ...
    # Some responses might include "fills": [...]
...
# =========================
# FIXED: Choose entry side based on HIGHER BIDS only
# =========================
def choose_entry_side(yes_bid: Optional[int], no_bid: Optional[int], mom: int) -> Optional[str]:
    ...
    """
    # Need both bids to compare
    if yes_bid is None or no_bid is None:
        return None
    
    # ALWAYS buy the side with HIGHER bids (the popular side)
    if yes_bid > no_bid:
        ...
        # Bids are equal - use momentum as tiebreaker
        if mom >= 1:
            print(f"📊 Bids equal, momentum up → Buying YES")
            return "yes"
        elif mom <= -1:
            print(f"📊 Bids equal, momentum down → Buying NO")
            return "no"
        else:
            print(f"📊 Bids equal, no momentum → No trade")
            return None
...
# Trading functions
# =========================
def place_order(action: str, side: str, ticker: str, contracts: int, price_cents: int, tif: Optional[str] = None, reduce_only: bool = False) -> Dict[str, Any]:
    payload = {
        "ticker": ticker,
...
def confirm_fill(resp: Dict[str, Any], side: str, contracts: int, timeout_s: float = 2.0) -> Tuple[bool, Optional[int], Optional[Dict[str, Any]]]:
    ...
    """
    if DRY_RUN or not ARM_LIVE:
        # assume “filled” at requested price
        p = resp.get("payload", {})
...
def main():
    ...
    while True:
        ...
        # =========================
        # FLAT: consider entry - FIXED to use higher bids
        # =========================
        if pos is None:
        ...
        # =========================
        # IN POSITION: AGGRESSIVE STOP LOSS
        # =========================
        # PnL in cents/contract based on MID proxy
        pnl_cents = (mid - pos.entry_yes_mid) if pos.side == "yes" else (pos.entry_yes_mid - mid)
        
        # Also calculate loss based on ACTUAL BID price (more accurate)
        current_bid = yb if pos.side == "yes" else nb
        if current_bid is not None:
            if pos.side == "yes":
                actual_loss_cents = pos.entry_fill_cents - current_bid
                actual_profit_cents = current_bid - pos.entry_fill_cents
            else:
                actual_loss_cents = pos.entry_fill_cents - current_bid
                actual_profit_cents = pos.entry_fill_cents - current_bid
        else:
            actual_loss_cents = 999  # Big number to trigger exit if no bid
            actual_profit_cents = 0

        print(f"[{now_hms()}] IN {pos.side.upper()} | Entry={pos.entry_fill_cents}¢ | Bid={current_bid}¢ | Mid PnL={pnl_cents:+d}¢ | Actual Loss={actual_loss_cents:.2f}¢ | Actual Profit={actual_profit_cents:.2f}¢ | Daily={money(daily_pnl)}")

        # CHECK STOP LOSS - MULTIPLE CONDITIONS
        exit_reason = None
        
        # Condition 1: Mid price stop loss
        if pnl_cents <= -STOP_LOSS_CENTS:
            exit_reason = f"MID_STOP_LOSS ({pnl_cents}¢)"
        
        # Condition 2: Actual bid price stop loss (MORE ACCURATE)
        ...
        # Condition 4: Actual profit take profit
        ...
        # ===== AGGRESSIVE EXIT - WILL DEFINITELY PULL YOU OUT =====
        ...
        # Try increasingly aggressive prices - WILL NOT STOP UNTIL SOLD
        for attempt in range(15):  # Many attempts
            # Calculate exit price - increasingly aggressive
            if attempt == 0:
                px = exit_bid
                print(f"Attempt 1: Selling at bid {px}¢")
            elif attempt < 5:
                px = max(1, exit_bid - attempt)
                print(f"Attempt {attempt+1}: Selling at {px}¢ (more aggressive)")
            elif attempt < 10:
                px = max(1, exit_bid - 5)
                print(f"Attempt {attempt+1}: Selling at {px}¢ (very aggressive)")
            else:
                px = 1  # Fire sale - sell at absolute minimum
                print(f"🔥 FIRE SALE: Selling at 1¢ (any price)")
        ...
        if not exited:
            ...
            print("Emergency mode - trying one last time at 1¢")
            
            # One last desperate attempt
            resp = place_order("sell", pos.side, TICKER, pos.contracts, 1, tif="immediate_or_cancel", reduce_only=True)
            filled, avg_fill, _ = confirm_fill(resp, pos.side, pos.contracts, timeout_s=3.0)
            
            if filled:
                exited = True
                exit_fill_price = avg_fill if avg_fill is not None else 1
        ...
        exit_proceeds = pos.contracts * cents_to_usd(exit_fill_price)
